...
addForm.addEventListener("submit", function (e) {
  ...
  if(validateCourseTitle(formData) && validateCourseStream(formData) && validateCourseType(formData) && validateStudentFirstname(formData) && validateStudentLastname(formData)){
  if(selectedRow === null){
    addData(formData);
  }else {
    updateRow(formData);
...
function addData(data){
    ...
    c4.innerHTML = data.studentFirstN.replace(/\s/g,'');
    c5.innerHTML = data.studentLastN.replace(/\s/g,'');
    c6.innerHTML = `<a
...
function editRow(td) {
  ...
  document.querySelector("#studentFirstN").value = selectedRow.cells[3].innerHTML;
  document.querySelector("#studentLastN").value = selectedRow.cells[4].innerHTML;
};
...
function updateRow(formData) {
  ...
  selectedRow.cells[3].innerHTML = formData.studentFirstN.replace(/\s/g,'');
  selectedRow.cells[4].innerHTML = formData.studentLastN.replace(/\s/g,'');
};
...
function validateCourseType(formData) {
